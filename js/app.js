...
if(state.currentFile){await loadFileToFfmpeg(); if(needsTranscode(state.currentFile)){els.placeholder.style.display='none';els.placeholder.innerHTML='<p>Нет загруженного видео</p>';els.video.classList.add('active');els.video.style.display='';els.canvas.classList.remove('active');await autoTranscode(state.currentFile);}}
}catch(err){
console.error(err);
...
if(!needsTc){
const url=URL.createObjectURL(file);els.video.src=url;els.video.load();
...
els.placeholder.style.display='none';els.video.classList.add('active');els.video.style.display='';els.canvas.classList.remove('active');state.transcodedBlobUrl=URL.createObjectURL(blob);
...
async function transcodeToMp4(){if(!state.ffmpegReady||!state.currentFile)return;const input="input"+ext(state.currentFileName);const output="output_manual.mp4";setStatus("transcoding","🔄 Конвертация в MP4...");updateProgress(0);els.resultPanel.style.display="none";try{await state.ffmpeg.exec(["-i",input,"-c:v","libx264","-preset","fast","-crf","23","-c:a","aac","-b:a","128k","-movflags","+faststart","-y",output]);const data=await state.ffmpeg.readFile(output);const blob=new Blob([data.buffer],{type:"video/mp4"});const url=URL.createObjectURL(blob);els.video.src=url;els.placeholder.style.display='none';els.video.classList.add('active');els.video.style.display='';els.canvas.classList.remove('active');state.transcodedBlobUrl=url;state.isTranscoded=true;els.video.load();els.video.play().catch(e=>console.log('Autoplay blocked:',e));showResult(data,"video/mp4","transcoded.mp4");setStatus("ready","✅ Конвертация завершена");if(state.webglEnabled&&state.webglFilter){state.webglFilter.setVideo(els.video);state.webglFilter.start();}}catch(e){appendLog("Ошибка: "+e.message);setStatus("error","❌ Ошибка конвертации");}finally{resetProgress();}}
